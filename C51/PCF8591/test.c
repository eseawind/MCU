...
int main()
{
    ...
    while(1)
    {
        // A/D转换，选择模拟输入AIN0信道(电位器0的电压信号)，采样并转化为数字
        ADC_Send_Byte(PCF8591, 0x40); //发送控制字节
        ...
        // A/D转换，选择模拟输入AIN1信道(电位器1的电压信号)，采样并转化为数字
        ADC_Send_Byte(PCF8591, 0x41); //发送控制字节
        ...
        // D/A转换， 片选PCF8591，写入控制字符0X40，开启模拟输出通道AOUT
        //将数字信号digital0转换为模拟信号输出到AOUT上，可以看到LED的明暗变化
        DAC_Conversion(PCF8591, 0x40, digital0);
    }
