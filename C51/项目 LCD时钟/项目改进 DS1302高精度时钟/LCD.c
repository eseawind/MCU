...
char YEAR1 = 20, YEAR2 = 13, MONTH = 2, DAY = 17, WEEK = 5, HOUR = 12, MINUTE = 0, SECOND = 0;  // 年月日周时分秒
char ALARM_H = 6, ALARM_M = 5, ALARM_S = 4;  // 闹铃变量, POS为 8,9,10说明是修改闹钟变量
unsigned char code ALARM_DISPLAY[] = "   Alarm  Set   ";  // 16 bytes
...
void LCD_Op2()     // 增加键操作,人为操作，因此不考虑进位
{
    ...
    case 8:
        if(++ALARM_H >= 24)
            ALARM_H = 0;
        LCD_Set(0x41, ALARM_H);
        break;
    case 9:
        if(++ALARM_M >= 60)
            ALARM_M = 0;
        LCD_Set(0x44, ALARM_M);
        break;
    case 10:
        if(++ALARM_S >= 60)
            ALARM_S = 0;
        LCD_Set(0x47, ALARM_S);
        break;
    }
...
void LCD_Op3()    // 闹铃键操作
{
    if(POS < 8) // 初次按闹铃键，配置POS
    {
        LCD_W_Cmd(0X01);   // 清屏操作

        // POS需要重新配置，这里暂时用作变量索引，打印一行闹钟信息
        POS = 0;
        while(ALARM_DISPLAY[POS] != '\0')
        {
            LCD_W_Data(ALARM_DISPLAY[POS]);
            POS++;
            Delayms(2);
        }
    ...
    else if(++POS >= 11)   // 多次按闹铃键，则移动位置，时，分，秒 三个位置
        POS = 8;
    ...
    case 8:
        ...
        break;       // ALARM_H
    case 9:
        LCD_Pos(0x45);   // ALARM_M
        break;
    case 10:
        LCD_Pos(0x48);   // ALARM_S
        break;
    }
}
